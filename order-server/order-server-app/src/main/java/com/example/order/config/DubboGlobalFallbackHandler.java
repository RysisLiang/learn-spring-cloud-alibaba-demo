    ...
    public void init() {
        ...
        DubboAdapterGlobalConfig.setProviderFallback(((invoker, invocation, ex) -> {
            ...
            if (ex instanceof DegradeException) { // 熔断异常的返回处理
                ...
                throw new RpcException("user-server-1 服务器熔断处理");
            } else if (ex instanceof FlowException) { // 降级异常的返回处理
                return AsyncRpcResult.newDefaultAsyncResult(new RuntimeException("user-server-1 服务器处理不过来啦"), invocation);
            }

            Tracer.trace(ex);
            // 默认的处理
            return AsyncRpcResult.newDefaultAsyncResult(new RuntimeException("user-server-1 服务器未知异常"), invocation);
        }));
    }
